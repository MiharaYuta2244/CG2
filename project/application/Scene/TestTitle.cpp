...
void TestTitle::Initialize(const SceneContext& ctx) {
	ctx_ = ctx;
	...
	// ONを選んだ時にすべての機能をtrueにしてシーン遷移
	menu_->AddItem("ON", "ON.png", [this]() {
		TestFunction::GetInstance()->SetAllFunctions(true);
		RequestSceneChange("PlayTest");
	});
	// OFFを選んだ時にすべての機能をfalseにしてシーン遷移
	menu_->AddItem("OFF", "OFF.png", [this]() {
		TestFunction::GetInstance()->SetAllFunctions(false);
		RequestSceneChange("PlayTest");
	});
	...
	commonData_->clearTime = 0.0f;
	commonData_->killCount = 0;
	commonData_->totalDeathCount = 0;
	commonData_->totalDamageCount = 0;
}
