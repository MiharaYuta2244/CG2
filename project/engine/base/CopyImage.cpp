...
void CopyImage::CreateRootSignature() {
...
void CopyImage::CreateGraphicsPipeline(DirectXCommon* dx) {
	...
	// シェーダーコンパイル
	IDxcBlob* vsBlob = DirectXUtils::CompileShader(L"resources/shaders/CopyImage.VS.hlsl", L"vs_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
	assert(vsBlob != nullptr);
	IDxcBlob* psBlob = DirectXUtils::CompileShader(L"resources/shaders/CopyImage.PS.hlsl", L"ps_6_0", dxcUtils_.Get(), dxcCompiler_.Get(), includeHandler_.Get());
...
void CopyImage::Initialize(DirectXCommon* dx) {
	// シェーダコンパイラ初期化
